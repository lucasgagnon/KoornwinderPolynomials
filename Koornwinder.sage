...
class AlcoveWalkTableaux(ClonableList):
    ...
    def pretty_print(self, compression = False):
        r"""
        Pretty print the argument in an ascii_art style.  If compression option is true, prints pipe "|"
...
class _CF_index_class(ClonableElement):
    ...
    def n(self):
        r"""
        returns initialization parameter n
        """
        return(self._n)
...
def KoornwinderPolynomialRing(n, ground_ring = None, verbose = True):
    ...
    R.inject_variables(verbose = verbose)
    ...
    KPR.inject_variables(verbose = verbose)
    return(KPR)
...
def Laurent_divided_difference(i, f):
    ...
    if i < 0 or i > n:
        raise(ValueError(f"Laurent divided difference passed illegal index {i}"))
    elif i == 0:
        for e in dd:
            ee = list(e)
            if ee[0] >= 0:
...
def Laurent_T(i, f):
    ...
    sqrt_tn = scalars[3]
    if i < 0 or i > n:
        raise(ValueError(f"Laurent T passed illegal index {i}"))
    if i == 0:
        a = KPR(sqrt_q*sqrt_t0*sqrt_u0^(-1))
        b = KPR(sqrt_q*sqrt_t0*sqrt_u0)
        return(KPR(sqrt_t0) - sqrt_t0^(-1) * (xv[0] - a) * (xv[0] - b) * Laurent_divided_difference(0, f))
    elif i == n:
        a = sqrt_tn*sqrt_un^(-1)*xv[-1]
        b = sqrt_tn*sqrt_un*xv[-1]
        return(KPR(sqrt_tn) - sqrt_tn^(-1) * (1 - a) * (1 - b) * Laurent_divided_difference(n, f))
    else:
        return(KPR(sqrt_t) - (sqrt_t^(-1)*xv[i] - sqrt_t*xv[i-1]) * Laurent_divided_difference(i, f))



def _get_tu_params(KPR, wt):
    r"""
    gets t and u parameters for the coroot wt by computing which 
    simple root orbit wt belongs to.
    """
...
def _get_Y_wt(KPR, wt, mu = None):
    r"""
    Evaluates Y^wt at the character `mu' (default=0) as an element of 
...
def ev_Y(i, n, mu = None):
    r"""
    Evaluates Yi for n at 0 (if no mu is specified) or mu otherwise.
    Uses field of scalars Q [use _get_Y_wt to evaluate in a specific koornwinder polynomial ring]
    """
    if i < 1 or i > n:
        raise(ValueError(f"Y evaluation passed illegal index {i}"))
    exponent = (n+1)*[0]
    exponent[i-1] = 1
    if mu is None:
        mu = n*[0]
    KPR = KoornwinderPolynomialRing(n, ground_ring = QQ, verbose = False)
    return(_get_Y_wt(KPR, exponent, mu))
...
def C_eval(wt, mu = None):
    ...
    KPR = KoornwinderPolynomialRing(n, verbose = False)
...
class CF_function_space(CombinatorialFreeModule):
    ...
    def __init__(self, n, R = None):
        ...
        if R == None:
            R = QQ
        self._KPR = KoornwinderPolynomialRing(n, R, verbose = False)
...
def KoornwinderAlcoveWalkFormula(mu, z = None):
    r"""
...
def greedy_step(mu):
    r"""
    Returns the next letter in the box greedy reduced word for the KoornwinderBoxDiagram mu,
    and the KoornwinderBoxDiagram obtained from removing that letter. 
    """
    n = len(mu)
...
def KoornwinderCreationFormula(mu):
    r"""
